...
impl ChannelInboundHandler for BizHandler {
    ...
    fn channel_active(&mut self, channel_handler_ctx: &mut ChannelInboundHandlerCtx) {
        ...
        let attr = channel_handler_ctx
            .channel()
            .get_attribute("User".to_string());
    ...
    fn channel_inactive(&mut self, channel_handler_ctx: &mut ChannelInboundHandlerCtx) {
        ...
        println!(
            "远端断开连接： Inactive: channel_id : {}",
            channel_handler_ctx.channel().id()
        )
    }
...
impl ChannelInboundHandler for Decoder {
    ...
    fn channel_active(&mut self, channel_handler_ctx: &mut ChannelInboundHandlerCtx) {
        ...
        channel_handler_ctx
            .channel()
            .set_attribute("User".to_string(), Box::new("lgphp".to_string()));
        println!(
            "解码 Handler --> : channel_active 新连接上线: {}",
            channel_handler_ctx.channel().remote_addr().unwrap()
        );
    ...
    fn channel_read(
        ...
        let buf = message.downcast_mut::<ByteBuf>().unwrap();
...
impl ChannelInboundHandler for InboundExceptionHandler {
    ...
    fn channel_active(&mut self, _channel_handler_ctx: &mut ChannelInboundHandlerCtx) {}

    fn channel_inactive(&mut self, _channel_handler_ctx: &mut ChannelInboundHandlerCtx) {}

    fn channel_read(
        &mut self,
        _channel_handler_ctx: &mut ChannelInboundHandlerCtx,
        _message: &mut dyn Any,
    ) {
    }
    ...
    fn channel_exception(
        ...
        let ch = channel_handler_ctx.channel();
        ...
        if error.kind == ErrorKind::TimedOut {
            println!(
                ...
                ch.id(),
                format!(
                    "{} ms 没有读到数据！ , error_message:{}",
                    ch.read_idle_timeout_ms(),
                    error.message
                )
            );
            ch.close()
...
impl ChannelOutboundHandler for Encoder {
    ...
    fn channel_write(
        ...
        buf.write_u32_be((1 + re.as_bytes().len()) as u32).unwrap();
        buf.write_string_with_u8_be_len(re).unwrap();
...
fn main() {
    ...
    new_default_event_loop_group
        .execute(|| println!(" default_event_loop  execute Task ..... is here"));
